...
class RequerimientoApiView(APIView):
    ...
    def post(self, request):
        ...
        if token:
            usuario_reporta_id = request.user.id
        else:
            return Response('El token no es válido', status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            created = Requerimiento.objects.create(
                codigo=codigo,
            ...
            serializer = RequerimientoSerializer(created)

            return Response(serializer.data, status=status.HTTP_200_OK)

        except Exception as e:
            return Response('Error creando requerimiento. ' + e.__str__(), status=status.HTTP_400_BAD_REQUEST)

    def get(self, request):
        requerimientos = Requerimiento.objects.all()
        serializer = RequerimientoSerializer(requerimientos, many=True)

        return Response(serializer.data, status=status.HTTP_200_OK)


# Las vistas para obtener y refrescar tokens
...
def mi_api(request):
    return render(request, 'mi_api.html')
...
def reset_password(request):
    ...
    if request.method == 'POST':
        try:
            ...
            context = {
                'error': 'No se econtró información con los datos ingresados'}
...
def create_default():
    ...
    Usuario.objects.create(user=user, role=role, rut=rut,
...
def requerimiento_update(request, id):
    ...
    if request.method == 'POST':
        try:
            ...
            requerimiento.usuario_reporta = Usuario.objects.get(
                pk=usuario_reporta_id)
            requerimiento.area = Area.objects.get(pk=area_id)
            if usuario_asignado_id is not None and int(usuario_asignado_id) > 0:
                requerimiento.usuario_asignado = Usuario.objects.get(
                    pk=usuario_asignado_id)
